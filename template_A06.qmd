...
[https://github.com/spencermclaughlin2027/McLaughlin_Spencer_201_A06](https://github.com/spencermclaughlin2027/McLaughlin_Spencer_201_A06)
...
```{r}
new_list = c() #create empty new list
names = names(player_info) #gets you each player's initials in a list
for (i in names) {
  index = player_info[[i]] #index each player's stats
  new_list[[i]] = (index$points)/(index$games_played) #add to new list points/games played
}
new_list
```
...
```{r}
round_mat <- function(val) {
  return(round(val, -(nchar(val)-1))) #rounds to the correct amount of digits
}

apply(mat_rand, MARGIN = 1:2, FUN = round_mat) #applies to everything in the matrix
```
...
```{r}
temps <- max_temps[3:14] #gets the month columns from data frame
means <- function(x) {
  return(mean(x)) #returns the average
}

apply(temps, MARGIN = 2, means) #gets average of all columns
```
...
```{r}
CO <- max_temps |> 
  filter(state == "CO") #slicing to include CO

OH <- max_temps |> 
  filter(state == "OH") #slicing to only include OH

VA <- max_temps |> 
  filter(state == "VA") #slicing to only include VA
```
...
```{r}
CO_temps <- CO[3:14] #gets the months columns
medians <- function(x) {
  return(median(x)) #return median
}
med_temps <- apply(CO_temps, MARGIN = 1, medians) #get median of each year

max <- med_temps[which.max(med_temps)] #find max median temp
min <- med_temps[which.min(med_temps)] #find min median temp

max_year <- CO$year[which.max(med_temps)] #find year of max median temp
min_year <- CO$year[which.min(med_temps)] #find year of min median temp

paste0("The maximum median annual annual temperature of ", max, " occurred in ", max_year, " and the minimum annual temperature of ", min, " occurred in ", min_year, ".")
```
...
sum_of_digits <- function(number) {
  if (number == 0){ #if number is 0 return 0
    return(0)
    }
  else {
    add = number%%10 #last digit
    new_num = number%/%10 #number with last digit removed
    return(add + sum_of_digits(new_num)) #add last number with sum of rest of digits
    }
}
