...
for (i in names) {
  index = player_info[[i]]
  new_list[[i]] = (index$points)/(index$games_played) 
}
new_list
```
...
round_mat <- function(val) {
  return(round(val, -(nchar(val)-1)))
}

apply(mat_rand, MARGIN = 1:2, FUN = round_mat)
```
